...
int main(){
	...
	//cout<<"Spill Width: "<<spillWidth<<endl<<"Spill Height: "<<spillHeight<<endl;
	timeNow=time(0)-timeNow;
	cout<<"Average RGB Caclulated: "<<timeNow<<endl;

	timeNow=time(0);
	//stbi_write_jpg(getName().c_str(), imgWidth, imgHeight, 3, avgImage, 100);
	delete rgbImage;
	...
	for (unsigned int row = 1; row < height-1; ++row) {
		for (unsigned int col = 1; col < width-1; ++col) {
			...
			gradient[i] = sqrt(pow(Gx,2.0) + pow(Gy,2.0));	// Calculate gradient strength
			...
			edgeDir[i] = newAngle;		// Store the approximate edge direction of each pixel in one array
		}
	}
	timeNow=time(0)-timeNow;
	...
	for (unsigned int row = 1; row < height - 1; ++row) {
		for (unsigned long col = 1; col < width - 1; ++col) {
			i = (unsigned long)(row*width + col);
			if (image[i] == 255) {		// Check to see if current pixel is an edge
				/* Switch based on current pixel's edge direction */
				switch (edgeDir[i]) {
					case 0:
	...
	for(int imgRow=0; imgRow<imgHeight; ++imgRow){
		for(int imgCol=0; imgCol<imgWidth; ++imgCol){
			...
			if(val==0){
				//cout<<r<<" "<<g<<" "<<b<<endl;
				//cout<<"Error: Could not find image with similar RGB. Please consider expanding the database."<<endl;
				for(int row=0; row<avgSize; ++row){
					for (int col=0; col<avgSize; ++col){
						i = (unsigned long) (imgRow*avgSize+row)*width*3+3*(avgSize*imgCol+col);
	...
	//cout<<"Couldn't find a similar image for "<<errorCount<<" tiles."<<endl;
	name=getName();
	timeNow=time(0);
...
void suppressNonMax(int rowShift, int colShift, int row, int col, int dir){
	...
	int newCol = 0;
	bool edgeEnd = false;
	float* nonMax = new float[width*3];		// Temporarily stores gradients and positions of pixels in parallel edges
...
int readFile(string name){
	//image = stbi_load(name.c_str(), &width, &height, &bpp, 1);
